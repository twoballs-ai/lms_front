...
function StudentProfileSettings() {
  ...
  const submitForm = (e) => {
    ...
    try {
      axios
        .put(baseUrl + 'student/' + studentId + '/', _formData
          // ,{ headers: { Authorization: `Token da0d550bcc813a1b1cc6b905551cb11e3bf95046` } }
          , { headers: { "Content-Type": "multipart/form-data" } }
        )
        .then(response => {
        ...
        <Card.Body>
          <Form>
            <Form.Group className="mb-3" controlId="formBasicfull_name">
              ...
              <Form.Control value={studentData.full_name} name="full_name" onChange={handleChange} type="text" placeholder="Введите ваше ФИО" />
            </Form.Group>
            <Form.Group controlId="formFile" className="mb-3">
              <Form.Label>Добавить картинку портфолио.</Form.Label>
              <Form.Control name="student_image" type="file" onChange={handleFileChange} />
            </Form.Group>
            {studentData.previous_student_image &&
              <Image src={studentData.previous_student_image} rounded width="400" />
              // <img src={courseEditData.previous_course_image} width="400" />
            }
            <Form.Group className="mb-3" controlId="formBasicEmail">
              <Form.Label>email</Form.Label>
              <Form.Control value={studentData.email} name="email" onChange={handleChange} type="email" placeholder="Введите ваш email" />
            </Form.Group>
            <Form.Group className="mb-3" controlId="formBasicqualification">
              <Form.Label>username</Form.Label>
              <Form.Control value={studentData.username} name="username" onChange={handleChange} type="text" placeholder="Введите вашу username" />
            </Form.Group>
            <Form.Group className="mb-3" controlId="formBasicphone">
              <Form.Label>interested_categories</Form.Label>
              <Form.Control value={studentData.interested_categories} name="interested_categories" onChange={handleChange} type="text" placeholder="Введите имя interested_categories" />
            </Form.Group>
